function [time, U_plot]=LeakyIF_3(input_currents,duration,dt,increase_threshold,tau_adapt)
...
% Matlab does not need for us to "return" them in the code below. 
% In Python, we would need to return them. 
%_____________________________________________________________________
%
...
% 
%Based on Dr. Kevin Gurney material.
...
%           ("psc") and the one below ("I_0"). In Python, this would be a 1D
%           Numpy array.
%
%           Addition with respect to LeakyIF_2: 
%           input_currents.U_0=To reflect the idea that there is no true input current, membrane 
%                              potential perturbation of 0.3 V (300 mV).
%           If translated to Python, we do not need to create a Python class if we do not want to.
%           we can just add a keyword input argument to the function with a
%           default value of U_0=0.3.
%
...
%           
%           increase_threshold=In volts, increases after each spike.
%           tau_adapt=In seconds, controls the relaxation period of the
%           threshold
%Outputs:
...
%morphology and the maximum temporal resolution of the sinmulation, and thus are hardcoded
    ...
    if nargin<3
        ...
        %modify it. Suggest NOT to modify it to avoid weird effects.
    end
    ...
    %1.4 Refined parameters
      arp = 0.006;  %Absolute refactory period in seconds. Usally very small, < 10 ms
                    %No spikes are allowed during this period inmediately
                    %after a spike
      backgroundI=3e-9;%Noise input backgound in Amp, present all the time
                       %Caused by the background input from the extracellular medium 
      %Additions with respect to LeakyIF_2: 
      %
      if nargin<4
             increase_threshold=0.012;%0.012; %In volts, this is how much the threshold increses after a spike
             %This will make harder to initiate a spike after a previous spike+ the absolute refractory period window
      end
      if nargin<5
          tau_adapt=0.3;%0.3;%0.15; %That is how fast the adaptive threshold decays to the inital " theta" value
          %when no spike
      end      
